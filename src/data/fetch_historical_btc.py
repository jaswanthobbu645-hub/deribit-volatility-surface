...
OUTPUT_FILE = "C:/Users/jaswa/deribit_project/os.path.join(PROCESSED_DIR, 'BTC_surface_1y.csv')"
CHECKPOINT_FILE = "C:/Users/jaswa/deribit_project/historical_scrape/checkpoint.csv"
...
def black_scholes_delta(S, K, T, r, sigma, option_type):
    """Calculate Black-Scholes delta for European call or put."""
    if T <= 0:
        if option_type == 'call':
...
def get_historical_volatility(currency="BTC"):
    """Get 1 year of daily historical volatility."""
...
def get_instruments(currency="BTC", kind="option"):
    """Get all option instruments (active + expired)."""
...
def save_checkpoint(results, filepath):
    """Save results to CSV checkpoint."""
    if not results:
        return
...
def load_checkpoint(filepath):
    """Load results from checkpoint if it exists."""
...
def main():
    print("=" * 60)
    print("Deribit BTC Options Surface Builder")
    print("=" * 60)
    ...
    # Step 2: Get all option instruments
    print("\n[2/4] Fetching BTC option instruments...")
    ...
    # Step 3: Fetch BTC-PERPETUAL (underlying)
    print("\n[3/4] Fetching BTC-PERPETUAL as underlying...")
    perp_data = get_tradingview_chart_data("BTC-PERPETUAL", start_timestamp, end_timestamp, "1D")
    ...
    for instr in option_instruments:
        ...
        # Skip if expired before our window
